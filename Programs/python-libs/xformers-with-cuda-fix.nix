{
  ...
  transformers,
  timm,
  # Additional inputs for torch-bin compatibility
  config,
  cudaPackages,
#, flash-attn
}:
let
  # Handle both torch and torch-bin packages
  isTorchBin = !(torch ? cudaSupport);
  
  # For torch-bin, get CUDA info from config, for regular torch use torch attributes
  cudaSupport = if isTorchBin 
    then (config.cudaSupport or false)
    else torch.cudaSupport;
    
  cudaCapabilities = if isTorchBin 
    then (config.cudaCapabilities or [])
    else (torch.cudaCapabilities or []);
    
  # For torch-bin, use the global cudaPackages, for torch use torch.cudaPackages
  torchCudaPackages = if isTorchBin 
    then cudaPackages
    else (torch.cudaPackages or cudaPackages);
...
buildPythonPackage {
  ...
  stdenv = if cudaSupport then torchCudaPackages.backendStdenv else stdenv;

  buildInputs = lib.optionals cudaSupport (
    with torchCudaPackages;
    [
      # flash-attn build
  ...
  nativeBuildInputs = [
    ninja
    which
  ] ++ lib.optionals cudaSupport (with torchCudaPackages; [ cuda_nvcc ]);
